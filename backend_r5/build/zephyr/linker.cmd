...
MEMORY
{
    FLASH (rx) : ORIGIN = 0x0, LENGTH = (0*1K - 0x0 - 0x0)
    RAM (wx) : ORIGIN = 0x0, LENGTH = (32 * 1K)
    ATCM ( rw ) : ORIGIN = (0), LENGTH = (32768) BTCM ( rw ) : ORIGIN = (1090584576), LENGTH = (32768) RSC_TABLE ( rw ) : ORIGIN = (2702180352), LENGTH = (1048576) DRAM ( rw ) : ORIGIN = (2703228928), LENGTH = (14680064)
...
/DISCARD/ :
{
 ...
 .debug_sup 0 : { *(.debug_sup) }
    ...
    ATCM (NOLOAD) : { __ATCM_start = .; KEEP(*(ATCM)) KEEP(*(ATCM.*)) __ATCM_end = .; } > ATCM __ATCM_size = __ATCM_end - __ATCM_start; __ATCM_load_start = LOADADDR(ATCM); BTCM (NOLOAD) : { __BTCM_start = .; KEEP(*(BTCM)) KEEP(*(BTCM.*)) __BTCM_end = .; } > BTCM __BTCM_size = __BTCM_end - __BTCM_start; __BTCM_load_start = LOADADDR(BTCM); RSC_TABLE (NOLOAD) : { __RSC_TABLE_start = .; KEEP(*(RSC_TABLE)) KEEP(*(RSC_TABLE.*)) __RSC_TABLE_end = .; } > RSC_TABLE __RSC_TABLE_size = __RSC_TABLE_end - __RSC_TABLE_start; __RSC_TABLE_load_start = LOADADDR(RSC_TABLE); DRAM (NOLOAD) : { __DRAM_start = .; KEEP(*(DRAM)) KEEP(*(DRAM.*)) __DRAM_end = .; } > DRAM __DRAM_size = __DRAM_end - __DRAM_start; __DRAM_load_start = LOADADDR(DRAM);
    .last_section :
    {
      LONG(0xE015E015)
    } > RAM
